...
- 如果平台没有原生 SSH/TCP 映射，默认会尝试用内置 token 池自动创建 ngrok TCP 入口，然后把 ngrok 分配的公网 host/port 写入客户端配置。禁用方式：`AUTO_NGROK_PUBLIC=off`。
...
- 当平台外层映射不可控时，脚本会用 ngrok TCP 作为兜底公网入口；这属于用户态反向隧道，不依赖容器具备公网入站权限，但速度/稳定性取决于 ngrok 免费线路和 token 额度。

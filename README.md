...
- 默认不主动新建 ngrok 兜底，也不会自动使用已有 ngrok；脚本会先打开容器内 SSH，再等待并扫描环境变量、Zo/FRP 配置、进程、`/__substrate`、`/etc/zo`、`/run`、`/tmp`、`/var/log` 等平台状态/日志里的 SSH 命令、`frpc` 配置和公网入口。需要使用已有 ngrok 时手动加 `AUTO_USE_EXISTING_NGROK=on`，需要主动新建 ngrok 兜底时手动加 `AUTO_NGROK_PUBLIC=on`。
- Zo/Zcomputer 原生 SSH 的容器内端口通常是 `2288`，配置在 `/etc/zo/supervisor.conf` 的 `[program:ssh]`，`AuthorizedKeysFile=/__substrate/.ssh/authorized_keys` 且默认 key-only。公网地址来自平台登录态接口 `GET https://api.zo.computer/system-services/ssh-tunnel`，返回 `tcp_addr`；容器内的 `ZO_CLIENT_IDENTITY_TOKEN` / `ZO_HOST_SERVICE_JWT` 不能代替网页登录 access token。若要让脚本自动查询该入口，需传入 `ZO_ACCESS_TOKEN` 或 `ZO_ACCESS_COOKIE`。
- 如果已经先运行过 `ngrok-ssh-pool.sh`，只有显式设置 `AUTO_USE_EXISTING_NGROK=on` 时脚本才会识别 `tcp://host:port -> localhost:本地端口`，并把 `MUX_PORT` 改成 ngrok 正在转发的本地端口，让公网 ngrok 入口直接进入 VPNMux。
...
- 对 Zo/Zcomputer，脚本会优先识别原生 SSH 本地端口 `2288`。若拿到了平台 `tcp_addr`，会使用该公网入口并接管 `2288` 做 VPNMux；切换时会停止系统 supervisor 里的原生 `ssh`，恢复脚本会把它再拉起。
